...
function activate(context) {
  const refreshCommand = vscode.commands.registerCommand(
    "mallocFreeScaffold.refreshDocument",
    async () => {
...
async function processDocument(doc, fullRefresh, changes = []) {
  ...
  for (const lineNumber of targetLines) {
    ...
    if (directAlloc) {
      ...
      if (!existing) {
        edits.push(makeInsertEdit(doc, lineNumber, directAlloc.indent, directAlloc.varName, config.cleanupFunctionName));
      }
    ...
    if (config.detectOwnedReturnFunctions) {
      const callAlloc = parseOwnedFactoryCallAssignment(
        text,
        analysis.ownedFunctionNames
      );
      ...
      if (callAlloc) {
        ...
        if (!existing) {
          edits.push(makeInsertEdit(doc, lineNumber, callAlloc.indent, callAlloc.varName, config.cleanupFunctionName));
        }
      }
    }
  }
...
function getConfig() {
  const cfg = vscode.workspace.getConfiguration("mallocFreeScaffold");
    ...
    if (!current) {
      if (!pending) {
        ...
        if (!trimmed || trimmed.startsWith("//")) {
          // keep waiting for opening brace
...
function parseDirectAllocationLine(text, allocatorFunctionsOrRegex, regexAlreadyBuilt = false) {
  ...
  const match = trimmed.match(
    new RegExp(
      String.raw`^(?:[A-Za-z_]\w*(?:\s+|\s*\*+\s*))+?([A-Za-z_]\w*)\s*=\s*(?:\([^)]*\)\s*)?(?:${allocatorRegex.source.slice(2, -2)})\s*\(`
    )
  ) || trimmed.match(
    new RegExp(
      String.raw`^([A-Za-z_]\w*)\s*=\s*(?:\([^)]*\)\s*)?(?:${allocatorRegex.source.slice(2, -2)})\s*\(`
    )
  );
...
function parseOwnedFactoryCallAssignment(text, ownedFunctionNames) {
  ...
  const match = trimmed.match(
    ...
    new RegExp(String.raw`^([A-Za-z_]\w*)\s*=\s*(${names})\s*\(`)
  );
...
function findScaffoldCleanupBelow(doc, startLine, varName, cleanupFunctionName, maxLine) {
  const pattern = new RegExp(
    `^\\s*${escapeRegex(cleanupFunctionName)}\\s*\\(\\s*${escapeRegex(varName)}\\s*\\)\\s*;\\s*$`
  );
...
function makeInsertEdit(doc, lineNumber, indent, varName, cleanupFunctionName) {
  return {
    kind: "insert",
